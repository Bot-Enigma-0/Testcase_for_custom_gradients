...
drag, del_L__del_U_autodiff = jax.value_and_grad(calc_drag, argnums=0)(U, x, y, node_ids)
grad_U_time = time.time()
print(f"J_value: {drag}")
...
grad_del_L__del_x, grad_del_L__del_y = [jax.grad(calc_drag, argnums=i)(U, x, y, node_ids) for i in (1, 2)];grad_del_L__del_z = np.zeros_like(grad_del_L__del_x)
